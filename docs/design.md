...
Results separate `machine_status`, `review_status`, `verification_status` and `formal_status`. Input binding includes the PR head/base, upstream commit, workspace/environment/policy digests and run ID/attempt. Current implementation uses `verified`, `review_pending`, `failed` and `not_run`; finer conditional/unsupported/infrastructure classifications remain explicit errors where not classified. No missing or unknown result grants a pass.
